...
export class Garage {
    ...
    enableHandlers = () => {
        this.root.addEventListener("click", (event) => {
            if (event.target instanceof HTMLElement && event.target.closest(".garage__item")) {
                const id = +event.target.closest(".garage__item").id
                if (event.target.textContent === BUTTON.REMOVE_CAR) this.handleRemoveCar(id)
                if (event.target.textContent === BUTTON.SELECT_CAR) this.handleSelectCar(id)
                if (event.target.textContent === BUTTON.START_CAR) this.handleStartCar([id])
                if (event.target.textContent === BUTTON.STOP_CAR) this.handleStopCar([id])
            }
    ...
    stopArrayRace(idArr: number[], distanceData: EngineData[]) {
        idArr.forEach((id, i) => {
    ...
    setWinner(id: number, time: number) {
        this.controller.winnersController.setWinner(id, time)
    ...
    handleStopCar = async (arrID: number[]) => {
        ...
        const distanceData = await Promise.all(promisesStopData)
        this.setIsLoadingButtons(arrID, BUTTONS.STOP, false)
        this.stopArrayRace(arrID, distanceData)
        this.pagination.toggleVisiblePagination(false)
    }
